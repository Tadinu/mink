...
import bisect
from typing_extensions import Callable, Tuple, Optional
import copy
...
def native_rollout(
  ...
  models = [model] * nsample
  # Set the initial states of all the tops, placing them on a grid
  mj.mj_resetDataKeyframe(model, data, 0)

  # Run the rollout
  datas = [copy.copy(data) for _ in range(nsample)]
  nstep = int(horizon / model.opt.timestep)
  # [nsample or 1] x [nstep or 1] x ncontrol
  ctrls = np.array([policy.action(data.time) for policy in policies]).reshape(nsample, 1, policies[0].naction)
  ...
  state, sensordata = mj_rollout.rollout(models, datas, initial_states,
                                         control=np.tile(ctrls, (1, nstep, 1)),
                                         nstep=nstep)
  return [reward(model, data) for data in datas]


def get_state(model, data, nsample=1):
...
class Planner:
  ...
  def __init__(
      ...
      noise_scale: float,
      nimprove: int,
      customize_initial_states: Optional[Callable] = None,
      interp: str = "zero",
      limits: bool = True,
  ):
    """Initialize planner.
    ...
    self._use_native_rollout = True
    self._initial_states = None
    self._customize_initial_states = customize_initial_states
    self.prep_init_states()

  def prep_init_states(self):
    if self._use_native_rollout:
      # [initial_states]: [nsample or 1] x nstate
      self._initial_states = get_state(self._model, self._data, self._nsample)
      if self._customize_initial_states:
        self._customize_initial_states(self._initial_states, self._nsample)
  ...
  def improve_policy(
    ...
    for _ in range(self._nimprove):
      ...
      if self._use_native_rollout:
        # noisy policy
        policies = [self.policy.noisy_copy(self._noise_scale)] * self._nsample
        self.prep_init_states()
        rewards = native_rollout(self._model, self._data,
                                 self._initial_states,
                                 self._reward, policies, self._nsample, self._horizon)
      else:
