...
_XML = _HERE / "planar_robot" / "scene.xml"
...
if __name__ == "__main__":
    ...
    collision_pairs = [
        (hand_geoms, ["floor", "obstacle_0", "obstacle_1"]),
    ]
    ...
    with mujoco.viewer.launch_passive(
        ...
        mink.move_mocap_to_frame(model, data, "target_mocap", "hand", "site")

        rate = RateLimiter(frequency=500.0, warn=False)
        while viewer.is_running():
            # Update task target.
            T_wt = mink.SE3.from_mocap_name(model, data, "target_mocap")
